...
RSpec.describe 'index.html' do
  ...
  context 'within <body>' do
    it 'contains a <header> tag' do
      ...
      'No closing </header> tag was found'
    end
    ...
    context 'within <main>' do
      it 'contains three separate <section> elements' do
        ...
        expect(section.length).to be >= 3,
        'Page should contain 3 sets of section tags'
      end
      ...
      it 'contains a <figure> tag within the first <section>' do
        ...
        expect(figure).to_not be_nil,
                              'No <figure> tag found within the first section'
        expect(html_file_contents).to include('</figure>'),
        'No closing </figure> tag was found'
        figcaption = figure.children.select { |ch| ch.name == 'figcaption' }[0]

        expect(figcaption).to_not be_nil,
                                  'No <figcaption> tag found within the first section'
        expect(html_file_contents).to include('</figcaption>'),
        'No closing </figcaption> tag was found'
      end
      ...
      it 'contains an <article> tag within the first <section>' do
        ...
        expect(article).to_not be_nil,
                               'No <article> tag found within the first section'
        expect(html_file_contents).to include('</article>'),
        'No closing </article> tag was found'
      end
      ...
      it 'contains a <figure> tag within the second <section>' do
        ...
        expect(figure).to_not be_nil,
                              'No <figure> tag found within the second section'
        expect(html_file_contents).to include('</figure>'),
        'No closing </figure> tag was found'
        figcaption = figure.children.select { |ch| ch.name == 'figcaption' }[0]

        expect(figcaption).to_not be_nil,
                                  'No <figcaption> tag found within the second section'
        expect(html_file_contents).to include('</figcaption>'),
        'No closing </figcaption> tag was found'
      end
    end
    ...
    it 'contains a <footer> tag' do
      ...
      'No closing </footer> tag was found'
    end
    ...
    context 'within <footer>' do
      it 'contains an <address> tag' do
        ...
        expect(html_file_contents).to include('</address>'),
        'No closing </address> tag was found'
      end

      it 'contains a <details> tag' do
        details = parsed_html.search('details')[0]
        expect(details).to_not be_nil, 'No <details> tag was found'
        expect(html_file_contents).to include('</details>'),
        'No closing </details> tag was found'
      end
    end
  end
end
